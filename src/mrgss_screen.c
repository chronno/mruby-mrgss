...
static mrb_value mrgss_screen_initialize(mrb_state* mrb, mrb_value self) {
    ...
    screen = mrb_malloc(mrb, sizeof(MR_Screen));
...
static mrb_value mrgss_screen_show(mrb_state* mrb, mrb_value self) {
    MR_Screen* cScreen = DATA_PTR(self);
...
static mrb_value mrgss_screen_update(mrb_state* mrb, mrb_value self) {
  ...
  mrb_funcall(mrb, game, "update", 0);
  if (!WindowShouldClose()) {
        glfwPollEvents();
        BeginDrawing();
            ClearBackground(RAYWHITE);
            mrb_funcall(mrb, game, "render", 0);
        EndDrawing();
    } else {
...
static mrb_value mrgss_screen_dispose(mrb_state* mrb, mrb_value self) {
  MR_Screen* cScreen = DATA_PTR(self);
  cScreen->disposed = TRUE;
  return mrb_nil_value();
}
...
void mrgss_screen_init(mrb_state *mrb) {
    ...
    mrb_define_method(mrb, mrb_screen, "disposed", mrgss_screen_dispose, MRB_ARGS_NONE());

    mrb_define_method(mrb, mrb_screen, "show", mrgss_screen_show, MRB_ARGS_NONE());
    mrb_define_method(mrb, mrb_screen, "update", mrgss_screen_update, MRB_ARGS_REQ(1));
}